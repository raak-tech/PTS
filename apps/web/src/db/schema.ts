...
export const users = pgTable("users", {
  ...
  role: text("role").notNull(), // 'client' | 'provider' | 'admin' | 'referrer'
...
/** B2B2C buyer organization (physio/sports-med clinic). */
...
/** Clinic-scoped authorization. Membership role, not users.role, defines scope. */
export const clinicMemberships = pgTable("clinic_memberships", {
  id: text("id").primaryKey(),
  clinicId: text("clinic_id").notNull(),
  userId: text("user_id").notNull(),
  role: text("role").notNull(), // 'referrer' | 'clinic_admin'
  createdAt: timestamp("created_at", { mode: "date", withTimezone: true }).notNull(),
});

/** Codes generated by a clinic member for patient enrollment/attribution. */
export const clinicEnrollmentCodes = pgTable("clinic_enrollment_codes", {
  id: text("id").primaryKey(),
  clinicId: text("clinic_id").notNull(),
...
/**
 * Patient-to-clinic attribution. Clinic sharing is separate from program consent
 * and can be withdrawn without ending the PTS program.
 */
export const clinicEnrollments = pgTable("clinic_enrollments", {
  id: text("id").primaryKey(),
  clinicId: text("clinic_id").notNull(),
  clientUserId: text("client_user_id").notNull(),
  referrerUserId: text("referrer_user_id"),
  enrollmentCode: text("enrollment_code"),
  consentSharedWithClinic: boolean("consent_shared_with_clinic").notNull().default(false),
  consentSharedAt: timestamp("consent_shared_at", { mode: "date", withTimezone: true }),
  consentWithdrawnAt: timestamp("consent_withdrawn_at", { mode: "date", withTimezone: true }),
  cohortLabel: text("cohort_label"),
  status: text("status").notNull().default("invited"),
  enrolledAt: timestamp("enrolled_at", { mode: "date", withTimezone: true }),
  firstActiveAt: timestamp("first_active_at", { mode: "date", withTimezone: true }),
  createdAt: timestamp("created_at", { mode: "date", withTimezone: true }).notNull(),
  updatedAt: timestamp("updated_at", { mode: "date", withTimezone: true }).notNull(),
});

/**
 * Validated outcome instruments captured at baseline (intake tail) and Week 6
 * (graduation). Counselor sees individual deltas; clinic sees aggregates only.
 */
export const outcomeMeasures = pgTable("outcome_measures", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull(),
  phase: text("phase").notNull(), // 'baseline' | 'week6'
  instrument: text("instrument").notNull(), // 'tsk11' | 'rts' | 'pseq' | 'phq2' | 'gad2' | 'pcs'
  score: integer("score").notNull(),
  rawJson: text("raw_json").notNull(), // JSON: item answers
  capturedAt: timestamp("captured_at", { mode: "date", withTimezone: true }).notNull(),
});

/**
 * Patient self-reported physio-exercise completion — a SEPARATE signal from PTS
 * program engagement. Always patient self-report, never device verified.
 */
export const physioSelfReports = pgTable("physio_self_reports", {
  id: text("id").primaryKey(),
  clientId: text("client_id").notNull(),
  dateIso: text("date_iso").notNull(),
  status: text("status").notNull(), // 'yes' | 'partly' | 'no'
