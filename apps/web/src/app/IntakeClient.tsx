...
function Radio({ name, value, checked, onChange, label }: any) {
  return (
    <label style={{
      ...
      border: checked ? `2px solid ${t.text}` : `2px solid ${t.border}`,
      backgroundColor: checked ? t.surfaceMuted : t.surface,
      color: t.text,
      fontSize: "clamp(13px, 2vw, 15px)", transition: "all 0.15s", fontWeight: checked ? 600 : 500
    }}>
      <input type="radio" name={name} value={value} checked={checked} onChange={onChange} style={{ accentColor: t.text, width: 18, height: 18, flexShrink: 0 }} />
      <span>{label}</span>
    </label>
  );
}
...
function Checkbox({ checked, onChange, label }: any) {
  return (
    <label style={{
      ...
      backgroundColor: checked ? t.surfaceMuted : t.surface,
      color: t.text,
      fontSize: "clamp(13px, 2vw, 15px)", fontWeight: checked ? 600 : 500
    }}>
      <input type="checkbox" checked={checked} onChange={onChange} style={{ accentColor: t.text, width: 18, height: 18, flexShrink: 0, marginTop: 2 }} />
      <span>{label}</span>
    </label>
  );
}
...
function Step1({ data, set }: any) {
  return (
    <div style={{ display: "grid", gap: 24 }}>
      <div>
        <p style={{ fontWeight: 600, fontSize: "clamp(14px, 3vw, 16px)", margin: "0 0 8px", color: t.text }}>What brought you here?</p>
        <p style={{ fontSize: "clamp(12px, 2vw, 13px)", color: t.textMuted, margin: 0, marginBottom: 10 }}>Choose the one that fits best.</p>
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      ...
      <div>
        ...
        <p style={{ fontSize: "clamp(12px, 2vw, 13px)", color: t.textMuted, margin: 0, marginBottom: 6 }}>Country or region — helps with time zones and context.</p>
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      <div>
        ...
        <p style={{ fontSize: "clamp(12px, 2vw, 13px)", color: t.textMuted, margin: 0, marginBottom: 8 }}>Be as specific as you like.</p>
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      <div>
        ...
        <p style={{ fontSize: "clamp(12px, 2vw, 13px)", color: t.textMuted, margin: 0, marginBottom: 8 }}>Optional — helps your counselor understand the full picture.</p>
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      <div style={{ background: "#fff8e1", border: "1px solid #ffe082", borderRadius: 10, padding: "12px 16px" }}>
        ...
        <p style={{ fontSize: "clamp(12px, 2vw, 13px)", color: t.textMuted, margin: "0 0 8px" }}>If any of these apply <strong>right now</strong>, please seek in-person medical care.</p>
        <ul style={{ margin: 0, paddingLeft: 20, display: "grid", gap: 4 }}>
          {redFlags.map(f => <li key={f} style={{ fontSize: "clamp(13px, 2vw, 14px)", color: t.textSecondary }}>{f}</li>)}
        </ul>
      </div>
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        if (body.error === "invalid") {
          throw new Error("Some answers look incomplete. Go back through the steps and try again.");
        }
        throw new Error(body.error ?? "Could not submit — please try again.");
      }
      localStorage.removeItem(STORAGE_KEY);
      const dest = data.hasRedFlags ? "/red-flags" : "/intake/complete";
  ...
  return (
    <div style={{ minHeight: "100vh", background: t.page, color: t.text, paddingBottom: 80 }}>
      {/* Header */}
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: t.surface, borderBottom: `1px solid ${t.borderLight}`, padding: "0 max(16px, 5vw)" }}>
        <div style={{ maxWidth: 720, margin: "0 auto", paddingTop: 16, paddingBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, fontSize: "clamp(13px, 2vw, 15px)" }}>
            <span style={{ fontWeight: 800, fontSize: "clamp(16px, 3vw, 18px)", letterSpacing: "-0.5px", color: t.text }}>PTS</span>
            <span style={{ color: t.textMuted, fontWeight: 600 }}>Step {step + 1} of {STEPS.length}</span>
          </div>
          <div style={{ height: 4, background: t.borderLight, borderRadius: 2 }}>
            <div style={{ height: 4, background: t.accent, borderRadius: 2, width: `${progress}%`, transition: "width 0.3s ease" }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "clamp(24px, 8vw, 40px) max(16px, 5vw)" }}>
        <div style={{ marginBottom: "clamp(20px, 6vw, 32px)" }}>
          <h1 style={{ fontSize: "clamp(20px, 6vw, 28px)", fontWeight: 700, margin: "0 0 8px", lineHeight: 1.2, color: t.text }}>{current.title}</h1>
          <p style={{ fontSize: "clamp(15px, 3vw, 17px)", color: t.textSecondary, margin: 0, lineHeight: 1.5, fontWeight: 500 }}>{current.subtitle}</p>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "clamp(28px, 8vw, 40px)", gap: 12, flexWrap: "wrap" }}>
          {step > 0 ? (
            <button onClick={() => setStep(s => s - 1)} style={{ padding: "12px 20px", borderRadius: 999, border: `1.5px solid ${t.border}`, background: t.surface, color: t.text, fontWeight: 600, fontSize: "clamp(13px, 2vw, 15px)", cursor: "pointer" }}>
              ← Back
            </button>
          ) : <div />}

          {step < STEPS.length - 1 ? (
            <button onClick={() => setStep(s => s + 1)} disabled={!canAdvance()} style={{ padding: "12px 24px", borderRadius: 999, border: "none", background: canAdvance() ? t.text : "#bbb", color: "white", fontWeight: 700, fontSize: "clamp(13px, 2vw, 15px)", cursor: canAdvance() ? "pointer" : "not-allowed" }}>
              Continue →
            </button>
          ) : (
            <button onClick={handleSubmit} disabled={!canAdvance() || submitting} style={{ padding: "12px 24px", borderRadius: 999, border: "none", background: canAdvance() && !submitting ? t.text : "#bbb", color: "white", fontWeight: 700, fontSize: "clamp(13px, 2vw, 15px)", cursor: canAdvance() && !submitting ? "pointer" : "not-allowed" }}>
