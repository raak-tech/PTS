...
export default function OneBoxScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) { const body = await res.json().catch(() => ({})); throw new Error((body as any).detail ?? (body as any).error ?? 'extraction failed'); }
      const result = await res.json();
      router.push({ pathname: '/(client)/intake/confirm', params: { result: JSON.stringify(result), freeText: text, segmentType: segmentType ?? 'other', round: String(round) } });
    } catch (e: any) {
  ...
  return (
    <Screen title={round > 1 ? 'A couple more details' : 'Tell us what is going on'} subtitle={round > 1 ? "Your counselor wants to make sure they really understand." : "Write freely — do not worry about structure."}>
